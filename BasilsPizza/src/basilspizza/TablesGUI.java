...
public class TablesGUI {
	...
	public JPanel tableSelect() {
		...
		MouseListener mouseListener = new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				populateTableOrdersTable(getSelectedTable());
			}
		};
	...
	public JPanel tableOrders() {
		...
		tableOrdersTableModel = new DefaultTableModel(new String[] {
				"Order ID", "Table ID", "Menu Item", "Quantity", "Price"
		}, 0);
	...
	public void populateTableOrdersTable(String tableId) {
		...
		/*
		ListSelectionModel lsm = tableList.getSelectionModel();
		lsm.clearSelection();
		...
		*/
		...
		//if (tableList.getModel().getSize() > 0) {
			...
			for (int i = 0; i < Database.getActiveTableOrderItemsArray().size(); i++) {
				...
				int orderId = Database.getActiveTableOrderItemsArray().get(i).getOrderId();
				String tableName = Database.getActiveTableOrderItemsArray().get(i).getTableName(); 
				int menuItemId = Database.getActiveTableOrderItemsArray().get(i).getMenuItemId();
				String menuItemName = Database.getActiveTableOrderItemsArray().get(i).getMenuItemName();
				int quantity = Database.getActiveTableOrderItemsArray().get(i).getQuantity();
				double menuItemPrice = Database.getActiveTableOrderItemsArray().get(i).getMenuItemPrice();

				System.out.println("TABLE ACTIVE: " + orderId + " " + tableName + " " + menuItemId + " " + menuItemName + " " + quantity);


				Object[] data = {orderId, tableName, menuItemName, quantity, menuItemPrice};



				tableOrdersTableModel.addRow(data);
	...
	/*
	private int getSelectedTablesTableIndex() {
		int row = tableList.getSelectedIndex();
		return row;
	}
	*/
