...
public class MenuGUI {
	...
	private JTable mainMenuTable, sidesMenuTable, drinksMenuTable, dessertsMenuTable;
	private DefaultTableModel mainMenuTableModel, sidesMenuTableModel, drinksMenuTableModel, dessertsMenuTableModel;
	...
	public JPanel mainMenu() {
		JPanel panelMainMenu = new JPanel(new BorderLayout());

		mainMenuTableModel = new DefaultTableModel(new String[] {
				"Menu Item ID", "Menu Item", "Price"
		}, 0);

		mainMenuTable = new JTable(mainMenuTableModel) { // Needs it's own JTable otherwise it doesn't work.
		...
		mainMenuTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent event) {
				...
				try {
					int row = mainMenuTable.getSelectedRow();
					String houseNumber = mainMenuTable.getModel().getValueAt(row, 2).toString();
					String address = mainMenuTable.getModel().getValueAt(row, 3).toString();
					String city = mainMenuTable.getModel().getValueAt(row, 4).toString();
		...
		panelMainMenu.add(jsp, BorderLayout.CENTER);
	...
	public String getSelectedMainMenuItem() {
		...
		int column = 0;
		int row = mainMenuTable.getSelectedRow();
		mainMenuItemName = mainMenuTable.getModel().getValueAt(row, 0).toString();

		System.out.println("SELECTED ROW " + mainMenuItemName);

		return mainMenuItemName;
	}

	public DefaultTableModel getMainMenuTableModel() {
